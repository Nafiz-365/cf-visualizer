...
export function SocialCards({
    ...
    const getDetailedStats = (subs: Submission[]) => {
        ...
        else if (favLang.includes('Java') && !favLang.includes('Script')) favLang = 'Java';

        return {
            favLang,
            favTag,
            avgDifficulty: difficultyCount > 0 ? Math.round(totalDifficulty / difficultyCount) : 0,
            successRate: subs.length > 0 ? ((okSubmissions.length / subs.length) * 100).toFixed(1) : '0.0',
            totalSolved: okSubmissions.length
        };
    };

    const detailedStats = useMemo(() => getDetailedStats(submissions), [submissions]);
    const opponentDetailedStats = useMemo(() => getDetailedStats(opponentSubmissions), [opponentSubmissions]);

    const cardTemplates = [
        { id: 'snapshot', label: 'SYS_OVERVIEW', icon: ImageIcon, color: '#00eeff' },
        { id: 'streak', label: 'ENGAGEMENT_CORE', icon: Flame, color: '#f97316' },
        { id: 'review', label: 'ANNUAL_METRICS', icon: Calendar, color: '#10b981' },
        { id: 'achievements', label: 'ELITE_PROTOCOLS', icon: Award, color: '#bc13fe' },
        { id: 'system_dump', label: 'SYSTEM_DUMP', icon: Database, color: '#ef4444' },
        { id: 'head_to_head', label: 'HEAD_TO_HEAD', icon: Swords, color: '#f43f5e' },
    ] as const;
    ...
    const handleDownload = async () => {
        ...
        if ((activeCard === 'snapshot' || activeCard === 'head_to_head') && (user.titlePhoto || user.avatar)) {
            try {
        ...
        if (activeCard === 'head_to_head' && opponentUser && (opponentUser.titlePhoto || opponentUser.avatar)) {
            try {
        ...
        const themeColor = cardTemplates.find(t => t.id === activeCard)?.color || '#00eeff';
        ...
        // Tech Corners Helper
        const drawCorners = (x: number, y: number, w: number, h: number, size: number, color: string) => {
        ...
        ctx.fillText(`STATUS: ONLINE`, 1100 - ctx.measureText(`STATUS: ONLINE`).width, 42);
        ...
        const drawFit = (text: string, x: number, y: number, maxW: number, align: CanvasTextAlign, sz: number, color: string) => {
            let s = sz;
            ctx.font = `bold ${s}px "Inter", sans-serif`;
            while (ctx.measureText(text).width > maxW && s > 7) { s -= 0.5; ctx.font = `bold ${s}px "Inter", sans-serif`; }
        ...
        if (activeCard === 'snapshot') {
            ...
            ctx.fillText(`[ RANK: ${(user.rank || 'UNRANKED').toUpperCase()} ]`, 80, 190);
            ...
            snapRows.forEach((row, i) => {
                ...
                if (row.val) {
                    drawFit(String(row.val), rightEdge, textY, COL_W - 100, 'right', 18, '#22d3ee');
                }
            });
            ctx.textAlign = 'left';
            ...
            ctx.beginPath();
            const endAngle = (Math.min(user.rating || 0, 4000) / 4000) * 2 * Math.PI;
            ...
            ctx.fillText('DAYS', 80 + ctx.measureText(`${streakDays}`).width + 20, 420);
            ctx.shadowBlur = 0;
            ...
            ctx.fillText(`CONTINUOUS SOLVING STREAK DETECTED. COGNITIVE ENGINE AT PEAK.`, 220, 525);
            ...
            drawReviewBox(440, 320, `+${Math.max(0, (user.rating || 800) - 1000)}`, '> RATING_DELTA');
            drawReviewBox(800, 320, `${ratingHistory.length}`, '> ROUNDS_SYNCED');
            ...
            const drawBadge = (x: number, y: number, name: string, desc: string, unlocked: boolean) => {
                ctx.fillStyle = unlocked ? 'rgba(188, 19, 254, 0.1)' : 'rgba(255, 255, 255, 0.02)';
                ...
                ctx.fillText(unlocked ? '[ VERIFIED ]' : '[ LOCKED ]', x + 370, y + 50);
            };

            drawBadge(80, 320, 'EXPERT_BURST', 'Peak performance >= 1600 RP', user.rating && user.rating >= 1600 ? true : false);
            drawBadge(600, 320, 'DP_ARCHITECT', 'Solved > 10 DP algorithms', solvedCount > 10);
            drawBadge(80, 460, 'CONTEST_SPEC', '5+ rated rounds completed', ratingHistory.length >= 5);
            drawBadge(600, 460, 'STREAK_FIRE', '7+ consecutive days active', streakDays >= 7);
            ...
            const drawDumpBox = (x: number, y: number, val: string, label: string) => {
            ...
            const stats = [
                { label: 'MAX RANK', val: (user.maxRank || 'N/A').toUpperCase() },
            ...
            stats.forEach((stat, idx) => {
                ...
                const col = idx % 4;
                drawDumpBox(startX + col * 260, startY + row * 120, stat.val, stat.label);
            });
            ...
            const SIDE_W = CANVAS_W * 0.30;           // 360px
            const TABLE_X = SIDE_W;
            const TABLE_W = CANVAS_W * 0.40;          // 480px
            ...
            ctx.fillText('⚔  B A T T L E   A N A L Y S I S  ⚔', CANVAS_W / 2, 34);
            ctx.shadowBlur = 0;

            // ── Avatar helper ──────────────────────────
            const drawAvatar = (img: HTMLImageElement | null, cx: number, cy: number, r: number, color: string) => {
                if (img) {
                    ctx.save();
                    ctx.beginPath();
            ...
            while (ctx.measureText(lHandle).width > SIDE_W - 10 && lHandle.length > 3) lHandle = lHandle.slice(0, -1);
            ctx.fillText(lHandle, L_CX, AVATAR_Y + AVATAR_R + 26);
            ctx.shadowBlur = 0;
            ctx.fillStyle = '#94a3b8';
            ctx.font = 'bold 10px "Courier New", monospace';
            ctx.fillText((user.maxRank || 'N/A').toUpperCase(), L_CX, AVATAR_Y + AVATAR_R + 44);
            ...
            while (ctx.measureText(rHandle).width > SIDE_W - 10 && rHandle.length > 3) rHandle = rHandle.slice(0, -1);
            ctx.fillText(rHandle, R_CX, AVATAR_Y + AVATAR_R + 26);
            ctx.shadowBlur = 0;
            ctx.fillStyle = '#94a3b8';
            ctx.font = 'bold 10px "Courier New", monospace';
            ctx.fillText((opponentUser.maxRank || 'N/A').toUpperCase(), R_CX, AVATAR_Y + AVATAR_R + 44);

            // ── Stats table ────────────────────────────
            const statRows = [
                { label: 'RATING',   left: String(user.rating || 0),              right: String(opponentUser.rating || 0) },
                { label: 'MAX RTG',  left: String(user.maxRating || 'N/A'),        right: String(opponentUser.maxRating || 'N/A') },
                { label: 'SOLVED',   left: String(detailedStats.totalSolved),      right: String(opponentDetailedStats.totalSolved) },
                { label: 'ACCURACY', left: `${detailedStats.successRate}%`,        right: `${opponentDetailedStats.successRate}%` },
                { label: 'WEAPON',   left: detailedStats.favLang,                  right: opponentDetailedStats.favLang },
                { label: 'DOMAIN',   left: detailedStats.favTag.toUpperCase(),     right: opponentDetailedStats.favTag.toUpperCase() },
                { label: 'ORG',      left: user.organization || 'INDIE',           right: opponentUser.organization || 'INDIE' },
                { label: 'LOC',      left: user.country || 'EARTH',               right: opponentUser.country || 'EARTH',
                                     leftFlag: userFlagImg, rightFlag: oppFlagImg },
            ];
            ...
            statRows.forEach((row, i) => {
                ...
                ctx.fillStyle = i % 2 === 0 ? 'rgba(6,182,212,0.05)' : 'rgba(249,115,22,0.05)';
                ...
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.moveTo(TABLE_X, rowY); ctx.lineTo(TABLE_X + TABLE_W, rowY);
                ctx.stroke();

                // Left value (right-aligned)
                const leftEdge = TABLE_X + COL_L_W - 14;
                if ((row as any).leftFlag) {
                    ctx.drawImage((row as any).leftFlag, leftEdge - 20, textY - 12, 20, 12);
                    drawFit(row.left, leftEdge - 26, textY, COL_L_W - 40, 'right', 15, '#06b6d4');
                } else {
                    drawFit(row.left, leftEdge, textY, COL_L_W - 20, 'right', 15, '#06b6d4');
                }
                ...
                if ((row as any).rightFlag) {
                    ctx.drawImage((row as any).rightFlag, rightEdge, textY - 12, 20, 12);
                    drawFit(row.right, rightEdge + 26, textY, COL_R_W - 40, 'left', 15, '#f97316');
                } else {
                    drawFit(row.right, rightEdge, textY, COL_R_W - 20, 'left', 15, '#f97316');
                }
            });
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(TABLE_X, tableBottom); ctx.lineTo(TABLE_X + TABLE_W, tableBottom);
            ctx.stroke();
        ...
        try {
            ...
            alert("Sorry, downloading failed. Please check the console for details.");
        } finally {
            setExporting(false);
        }
    };
        ...
        <div className="space-y-6 md:space-y-8 max-w-5xl mx-auto">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-xs md:text-sm text-muted-app font-medium max-w-2xl font-mono">
                    Export high-resolution cyberpunk data cards to share your Codeforces metrics. 
                    Optimized for 1200x630 resolution.
                </p>
            </div>
                ...
                <div className="lg:col-span-4 space-y-3">
                    ...
                    <div className="space-y-2 md:space-y-3">
                        {cardTemplates.map((template) => {
                            ...
                            const themeClass = 
                                template.id === 'snapshot' ? 'text-cyan-500 border-cyan-500/30 bg-cyan-500/10 shadow-[0_0_15px_rgba(6,182,212,0.15)]' :
                                template.id === 'streak' ? 'text-orange-500 border-orange-500/30 bg-orange-500/10 shadow-[0_0_15px_rgba(249,115,22,0.15)]' :
                                template.id === 'review' ? 'text-emerald-500 border-emerald-500/30 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.15)]' :
                                template.id === 'system_dump' ? 'text-red-500 border-red-500/30 bg-red-500/10 shadow-[0_0_15px_rgba(239,68,68,0.15)]' :
                                template.id === 'head_to_head' ? 'text-rose-500 border-rose-500/30 bg-rose-500/10 shadow-[0_0_15px_rgba(244,63,94,0.15)]' :
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        isActive
                                            ? themeClass
                                            : 'bg-[var(--bg-card)] border-[var(--glass-border)] text-muted-app hover:bg-[var(--glass-bg)] hover:text-text-app',
                                    )}
                                >
                                    ...
                                    {isActive && <Check size={14} className="relative z-10" />}
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <Button
                        ...
                        className="w-full h-10 md:h-12 rounded-xl text-[10px] uppercase font-black tracking-widest mt-4 md:mt-6 font-mono border border-[var(--glass-border)] relative overflow-hidden group hover:scale-[1.02] transition-transform text-white bg-slate-900"
                        isLoading={exporting}
                    >
                    ...
                    <div className="aspect-[1.9] w-full rounded-xl md:rounded-2xl overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.2)] dark:shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-slate-800 relative text-left bg-slate-950">
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {/* Radial Glow */}
                                <div className={cn(
                                    "absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))]",
                                    activeCard === 'snapshot' && 'from-cyan-400 via-transparent to-transparent',
                                    activeCard === 'streak' && 'from-orange-500 via-transparent to-transparent',
                                    activeCard === 'review' && 'from-emerald-500 via-transparent to-transparent',
                                    activeCard === 'system_dump' && 'from-red-500 via-transparent to-transparent',
                                    activeCard === 'achievements' && 'from-purple-500 via-transparent to-transparent'
                                )} />

                                {/* Tech Corners in HTML */}
                                <div className={cn(
                                    "absolute top-2 left-2 md:top-4 md:left-4 w-4 h-4 md:w-8 md:h-8 border-t-[1px] border-l-[1px] md:border-t-2 md:border-l-2",
                                    activeCard === 'snapshot' ? 'border-cyan-500/50' : 
                                    activeCard === 'streak' ? 'border-orange-500/50' : 
                                    activeCard === 'review' ? 'border-emerald-500/50' : 
                                    activeCard === 'system_dump' ? 'border-red-500/50' : 'border-purple-500/50'
                                )} />
                                <div className={cn(
                                    "absolute top-2 right-2 md:top-4 md:right-4 w-4 h-4 md:w-8 md:h-8 border-t-[1px] border-r-[1px] md:border-t-2 md:border-r-2",
                                    activeCard === 'snapshot' ? 'border-cyan-500/50' : 
                                    activeCard === 'streak' ? 'border-orange-500/50' : 
                                    activeCard === 'review' ? 'border-emerald-500/50' : 
                                    activeCard === 'system_dump' ? 'border-red-500/50' : 'border-purple-500/50'
                                ...
                                <div className={cn(
                                    "absolute bottom-2 left-2 md:bottom-4 md:left-4 w-4 h-4 md:w-8 md:h-8 border-b-[1px] border-l-[1px] md:border-b-2 md:border-l-2",
                                    activeCard === 'snapshot' ? 'border-cyan-500/50' : 
                                    activeCard === 'streak' ? 'border-orange-500/50' : 
                                    activeCard === 'review' ? 'border-emerald-500/50' : 
                                    activeCard === 'system_dump' ? 'border-red-500/50' : 'border-purple-500/50'
                                ...
                                <div className={cn(
                                    "absolute bottom-2 right-2 md:bottom-4 md:right-4 w-4 h-4 md:w-8 md:h-8 border-b-[1px] border-r-[1px] md:border-b-2 md:border-r-2",
                                    activeCard === 'snapshot' ? 'border-cyan-500/50' : 
                                    activeCard === 'streak' ? 'border-orange-500/50' : 
                                    activeCard === 'review' ? 'border-emerald-500/50' : 
                                    activeCard === 'system_dump' ? 'border-red-500/50' : 'border-purple-500/50'
                                        ...
                                        <div className="flex gap-0.5 md:gap-1">
                                            {[...Array(5)].map((_,i) => (
                                                <div key={i} className={cn(
                                                    "w-[2px] h-[2px] md:w-1 md:h-1 rounded-full",
                                                    activeCard === 'snapshot' ? 'bg-cyan-500/40' : 
                                                    activeCard === 'streak' ? 'bg-orange-500/40' : 
                                                    activeCard === 'review' ? 'bg-emerald-500/40' : 
                                                    activeCard === 'system_dump' ? 'bg-red-500/40' : 'bg-purple-500/40'
                                                )} />
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex-1 flex items-center justify-between relative z-10">
                                        <div className="flex flex-col justify-center space-y-2 sm:space-y-3 md:space-y-4 lg:space-y-6 w-full pr-2">
                                            ...
                                            <div className="grid grid-cols-2 gap-1 sm:gap-2 pt-1 sm:pt-2 md:pt-4 border-t border-cyan-500/20 w-full max-w-lg">
                                                {[
                                                    { label: 'RATING', val: user.rating || 0 },
                                                    { label: 'MAX RTG', val: maxRating },
                                                    { label: 'SOLVED', val: solvedCount },
                                                    { label: 'ACCURACY', val: `${detailedStats.successRate}%` },
                                                    { label: 'WEAPON', val: detailedStats.favLang },
                                                    { label: 'DOMAIN', val: detailedStats.favTag.toUpperCase() },
                                                    { label: 'ORG', val: user.organization || 'INDIE' },
                                                    { label: 'LOC', val: '', flag: user.country }
                                                ].map((stat, idx) => (
                                                    <div key={idx} className="flex justify-between items-center py-1 px-1.5 sm:px-2 border border-cyan-500/20 bg-cyan-950/30 rounded-sm">
                                                        <span className="text-[5px] sm:text-[7px] md:text-[9px] text-cyan-200/70 font-mono tracking-widest shrink-0">{stat.label}</span>
                                                        <div className="flex items-center gap-1 sm:gap-2 min-w-0 justify-end ml-2">
                                                            {stat.flag && countries.getAlpha2Code(stat.flag, 'en') && (
                                                                <img src={`https://flagcdn.com/w20/${countries.getAlpha2Code(stat.flag, 'en')?.toLowerCase()}.png`} alt="flag" className="w-2.5 h-1.5 sm:w-3 sm:h-2 md:w-4 md:h-3 rounded-[1px] object-cover shrink-0" crossOrigin="anonymous" />
                                                            )}
                                                            {stat.val && <span className="text-[6px] sm:text-[8px] md:text-xs font-bold font-mono text-cyan-400 leading-none truncate">{stat.val}</span>}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center justify-center pr-2 sm:pr-4 md:pr-12 shrink-0">
                                            <div 
                                                className="relative w-12 h-12 sm:w-16 sm:h-16 md:w-24 md:h-24 lg:w-32 lg:h-32 rounded-full flex items-center justify-center before:absolute before:-inset-[2px] md:before:-inset-[4px] before:rounded-full before:bg-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.3)]"
                                            >
                                                {/* The actual progress ring */}
                                                <div 
                                                    className="absolute -inset-[2px] md:-inset-[4px] rounded-full"
                                                    style={{
                                            ...
                                            <div className="text-center mt-2 md:mt-4 relative z-10">
                                                <p className="text-[5px] sm:text-[7px] md:text-[10px] uppercase text-white font-mono font-bold tracking-widest">PWR LEVEL</p>
                                                <p className="text-xs sm:text-sm md:text-xl lg:text-2xl font-black text-cyan-400 drop-shadow-[0_0_8px_rgba(6,182,212,0.8)]">{user.rating || 0}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {activeCard === 'review' && (
                                    <div className="flex-1 flex flex-col justify-center space-y-3 sm:space-y-4 md:space-y-6 relative z-10">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 md:gap-4 pt-2 md:pt-4">
                                            {[
                                                { label: '> TOTAL_SOLVED', val: solvedCount },
                                                { label: '> RATING_DELTA', val: `+${Math.max(0, (user.rating || 800) - 1000)}` },
                                                { label: '> ROUNDS_SYNCED', val: ratingHistory.length }
                                            ].map((stat, idx) => (
                                                <div key={idx} className="p-2 sm:p-4 md:p-6 bg-emerald-950/10 border border-emerald-500/30 rounded-md md:rounded-lg backdrop-blur-sm relative">
                                ...
                                {activeCard === 'achievements' && (
                                    <div className="flex-1 flex flex-col justify-center space-y-3 sm:space-y-4 md:space-y-6 relative z-10">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                                            {[
                                                { name: 'EXPERT_BURST', desc: 'Peak performance >= 1600 RP', unl: user.rating && user.rating >= 1600 ? true : false },
                                                { name: 'DP_ARCHITECT', desc: 'Solved > 10 DP algorithms', unl: solvedCount > 10 },
                                                { name: 'CONTEST_SPEC', desc: '5+ rated rounds completed', unl: ratingHistory.length >= 5 },
                                                { name: 'STREAK_FIRE', desc: '7+ consecutive days active', unl: streakDays >= 7 },
                                            ].map((badge, idx) => (
                                                <div key={idx} className={cn(
                                                    "p-2 md:p-4 rounded-md md:rounded-lg flex flex-col justify-center border backdrop-blur-sm relative overflow-hidden",
                                                    badge.unl ? "bg-purple-900/20 border-purple-500/40" : "bg-slate-900/40 border-slate-700/50"
                                                )}>
                                                    <div className="flex justify-between items-center mb-0.5 md:mb-1">
                                                        <p className={cn("text-[8px] sm:text-xs md:text-sm font-black font-mono", badge.unl ? "text-white drop-shadow-[0_0_6px_rgba(188,19,254,0.8)]" : "text-slate-500")}>
                                                            {badge.name}
                                                        </p>
                                                        <p className={cn("text-[6px] sm:text-[8px] md:text-[10px] font-black", badge.unl ? "text-purple-400" : "text-slate-600")}>
                                                            {badge.unl ? '[ VERIFIED ]' : '[ LOCKED ]'}
                                                        </p>
                                                    </div>
                                                    <p className={cn("text-[5px] sm:text-[7px] md:text-[10px] font-mono", badge.unl ? "text-purple-300" : "text-slate-600")}>
                                                        {badge.desc}
                                                    </p>
                                                </div>
                                            ))}
                                    ...
                                    <div className="flex-1 flex flex-col justify-center space-y-3 sm:space-y-4 md:space-y-6 relative z-10">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 pt-2 md:pt-4 border-t border-red-500/20">
                                            {[
                                                ...
                                                <div key={idx} className="p-1 sm:p-2 md:p-3 bg-red-950/20 border-l border-b border-red-500/30 overflow-hidden flex flex-col justify-center">
                                        ...
                                        {!opponentUser ? (
                                            <div className="flex flex-col items-center justify-center space-y-4 max-w-sm mx-auto w-full h-full">
                                                <div className="text-center space-y-2">
                                                    <h3 className="text-xl md:text-2xl font-black text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.6)]">ENTER CHALLENGER</h3>
                                                    <p className="text-[8px] md:text-[10px] text-slate-400 font-mono tracking-widest uppercase">Initiate Head-to-Head Protocol</p>
                                                </div>
                                                <form onSubmit={handleFetchOpponent} className="w-full space-y-3 relative z-50 pointer-events-auto">
                                                    <div className="relative">
                                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-rose-500/50" size={14} />
                                                        <input 
                                                            type="text" 
                                                    ...
                                                    <Button 
                                                        type="submit" 
                                                        disabled={isFetchingOpponent || !opponentInput.trim()}
                                                        isLoading={isFetchingOpponent}
                                                    ...
                                                    {opponentError && (
                                                        <p className="text-[10px] text-red-500 text-center font-mono flex items-center justify-center gap-1"><AlertCircle size={10}/> {opponentError}</p>
                                                    )}
                                                </form>
                                            </div>
                                        ) : (
                                            <div className="w-full h-full flex flex-col min-h-0">
                                                ...
                                                <div className="text-center pb-1 shrink-0">
                                                    <p className="text-[6px] sm:text-[8px] font-mono text-rose-400 tracking-[0.2em] uppercase">⚔ Battle Analysis ⚔</p>
                                                </div>
                                                {/* Stats Table */}
                                                <div className="flex-1 flex items-stretch min-h-0 overflow-hidden">
                                                    ...
                                                    <div className="flex flex-col items-center shrink-0 w-[30%]">
                                                        ...
                                                        <p className="text-[7px] sm:text-[9px] md:text-xs font-black text-cyan-400 drop-shadow-[0_0_5px_rgba(6,182,212,0.8)] w-full text-center leading-tight px-1">{user.handle}</p>
                                                        <p className="text-[5px] sm:text-[6px] md:text-[8px] text-cyan-200/70 uppercase font-mono leading-tight">{user.maxRank || 'N/A'}</p>
                                                    </div>

                                                    {/* Stats Columns */}
                                                    <div className="flex-1 flex flex-col justify-start min-w-0 px-1 overflow-hidden">
                                                        {[
                                                            { label: 'RATING', left: user.rating || 0, right: opponentUser.rating || 0 },
                                                            { label: 'MAX RTG', left: user.maxRating || 'N/A', right: opponentUser.maxRating || 'N/A' },
                                                            { label: 'SOLVED', left: detailedStats.totalSolved, right: opponentDetailedStats.totalSolved },
                                                            { label: 'ACCURACY', left: `${detailedStats.successRate}%`, right: `${opponentDetailedStats.successRate}%` },
                                                            { label: 'WEAPON', left: detailedStats.favLang, right: opponentDetailedStats.favLang },
                                                            { label: 'DOMAIN', left: detailedStats.favTag, right: opponentDetailedStats.favTag },
                                                            { label: 'ORG', left: user.organization || 'INDIE', right: opponentUser.organization || 'INDIE' },
                                                            { label: 'LOC', left: user.country || 'EARTH', right: opponentUser.country || 'EARTH', leftFlag: user.country, rightFlag: opponentUser.country },
                                                        ].map((row, i) => (
                                                            <div key={i} className={cn("flex items-center border-b border-slate-700/40 py-[2px] sm:py-[3px] min-h-0", i % 2 === 0 ? 'bg-cyan-500/5' : 'bg-orange-500/5')}>
                                                                <div className="w-[38%] flex items-center justify-end pr-1">
                                                                    <span className="text-cyan-400 font-bold text-[5px] sm:text-[7px] md:text-[9px] font-mono leading-tight truncate">{String(row.left)}</span>
                                                                    {(row as any).leftFlag && countries.getAlpha2Code((row as any).leftFlag, 'en') && (
                                                                        <img src={`https://flagcdn.com/w20/${countries.getAlpha2Code((row as any).leftFlag, 'en')?.toLowerCase()}.png`} alt="flag" className="w-2 h-1.5 sm:w-3 sm:h-2 rounded-sm object-cover ml-1 shrink-0" crossOrigin="anonymous" />
                                                                    )}
                                                                </div>
                                                                <span className="text-slate-500 text-[4px] sm:text-[6px] md:text-[8px] font-mono text-center w-[24%] shrink-0 tracking-widest leading-tight">{row.label}</span>
                                                                <div className="w-[38%] flex items-center justify-start pl-1">
                                                                    {(row as any).rightFlag && countries.getAlpha2Code((row as any).rightFlag, 'en') && (
                                                                        <img src={`https://flagcdn.com/w20/${countries.getAlpha2Code((row as any).rightFlag, 'en')?.toLowerCase()}.png`} alt="flag" className="w-2 h-1.5 sm:w-3 sm:h-2 rounded-sm object-cover mr-1 shrink-0" crossOrigin="anonymous" />
                                                                    )}
                                                                    <span className="text-orange-400 font-bold text-[5px] sm:text-[7px] md:text-[9px] font-mono leading-tight truncate">{String(row.right)}</span>
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                    ...
                                                    <div className="flex flex-col items-center shrink-0 w-[30%]">
                                                        ...
                                                        <p className="text-[7px] sm:text-[9px] md:text-xs font-black text-orange-400 drop-shadow-[0_0_5px_rgba(249,115,22,0.8)] w-full text-center leading-tight px-1">{opponentUser.handle}</p>
                                                        <p className="text-[5px] sm:text-[6px] md:text-[8px] text-orange-200/70 uppercase font-mono leading-tight">{opponentUser.maxRank || 'N/A'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                ...
                                <div className="flex justify-between items-center text-[6px] sm:text-[8px] md:text-[10px] font-bold tracking-widest relative z-10 pt-2 md:pt-4 border-t border-slate-800">
                                     <span className={cn(
                                        activeCard === 'snapshot' ? 'text-cyan-500 drop-shadow-[0_0_3px_rgba(6,182,212,0.8)]' : 
                                        activeCard === 'streak' ? 'text-orange-500 drop-shadow-[0_0_3px_rgba(249,115,22,0.8)]' : 
                                        activeCard === 'review' ? 'text-emerald-500 drop-shadow-[0_0_3px_rgba(16,185,129,0.8)]' : 
                                        activeCard === 'system_dump' ? 'text-red-500 drop-shadow-[0_0_3px_rgba(239,68,68,0.8)]' : 
                                        activeCard === 'head_to_head' ? 'text-rose-500 drop-shadow-[0_0_3px_rgba(244,63,94,0.8)]' : 'text-purple-500 drop-shadow-[0_0_3px_rgba(188,19,254,0.8)]'
                                    )}>
