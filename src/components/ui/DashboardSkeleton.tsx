...
export function DashboardSkeleton({ handle }: { handle?: string }) {
    return (
        <div className="min-h-screen flex" style={{ background: 'var(--bg-app)' }}>
            {/* Skeleton sidebar */}
            <aside className="hidden md:flex w-20 lg:w-64 border-r flex-col pt-8 px-3 space-y-2"
            ...
            <main className="flex-1 md:ml-20 lg:ml-64">
                {/* Skeleton header */}
                <div className="sticky top-0 z-40 px-4 md:px-8 py-4 border-b flex items-center gap-4"
                     style={{ background: 'var(--bg-app)', borderColor: 'var(--glass-border)' }}>
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 pb-24 space-y-8">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                        <div className="xl:col-span-8 rounded-[1.75rem] p-8 space-y-4"
                        ...
                        <div className="xl:col-span-4 grid grid-cols-2 xl:grid-cols-1 gap-4">
                            {[0, 1].map((i) => (
                                <div key={i} className="rounded-[1.75rem] p-6 space-y-3"
                                     style={{ background: 'var(--bg-card)', border: '1px solid var(--glass-border)' }}>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                        {Array.from({ length: 5 }).map((_, i) => (
                            <div key={i} className="rounded-[1.75rem] p-5 space-y-3 min-h-[120px]"
                                 style={{ background: 'var(--bg-card)', border: '1px solid var(--glass-border)' }}>
                    ...
                    {/* Chart area */}
                    <div className="rounded-[1.75rem] p-6 h-72"
                         style={{ background: 'var(--bg-card)', border: '1px solid var(--glass-border)' }}>
                    ...
                    <div className="text-center pt-4 space-y-1">
                        <p className="text-sm font-display font-medium animate-pulse"
                           style={{ color: 'var(--text-main)' }}>
                            Analyzing{handle ? ` ${handle}'s` : ''} profile intelligence…
                        </p>
